...
jobs:
  build:
    ...
    steps:
      - name: Issue Recorder
        uses: noraworld/issue-recorder@v0.2.2
        with:
          mode: file
...
| ~~`title_prefix`~~ **[DEPRECATED]** | ~~Specify additional letters or emojis appearing at the beginning of the title in an issue~~<br><br>Use `title_prefix_for_issue` instead                                                                                             | ~~`issue`~~     | ~~String~~ | ~~`""`~~                                                      | ~~`✅`~~                                                                      |
